...
public class VBankService {
    private static String wholeNumbersInsult = "Vbucks are whole numbers, you noob! (1, 2, 3, 4, 5, 6, 7, 8, 9, 0)\nTry again!";
    private static String notEnoughVbucksInsult = "You don't have enough Vbucks you noob!\nTry again!";

    static Scanner scanner = new Scanner(System.in);

    public static void depositVbucks(int playerId, int accountId) {
        int amount = -1;
        System.out.println("Please enter a valid Vbucks amount to deposit:");
        while (amount < 0) {
    ...
    public static void withdrawVbucks(int playerId, int accountId) {
        int amount = -1;
        System.out.println("Please enter a valid Vbucks amount to withdraw:");
        while (amount < 0) {
            if (!scanner.hasNextInt()) {
                System.out.println(wholeNumbersInsult);
                scanner.nextLine();
    ...
    public static void giftVbucks(int playerId, int gifterAccount) {
        int amount = -1;
        System.out.println("Please enter a valid Vbucks amount to gift:");
        while (amount < 0) {
            if (!scanner.hasNextInt()) {
                System.out.println(wholeNumbersInsult);
                scanner.nextLine();
        ...
        while (giftedAccount < 0) {
            System.out.println("Please select a valid Vbucks account to receive your generous gift");
            boolean accountExists = VbucksAccountDao.checkVbucksAccountExists(giftedAccount);
            if (!accountExists) {
                System.out.println("That account doesn't exist you noob!\nTry again!");
                scanner.nextLine();
                continue;
            }
    ...
    public static void createJointAccount(int accountId) {
        int jointAccount = -1;
        System.out.println("Please enter a valid Vbucks account to link:");
        while (jointAccount < 0) {
            if (!scanner.hasNextInt()) {
                System.out.println("Invalid account, you noob! Try again!");
                scanner.nextLine();
                continue;
            }
            ...
            if (!jointAccountExists) {
                System.out.println("That account doesn't exist, you noob! And no, I'm not gonna give you options to choose from! #securitythreats #noob\nTry again!!");
                scanner.nextLine();
                jointAccount = -1;
                continue;
            }
    ...
    public static void closeAccount(int accountId) {
        String confirmClose = "";
        System.out.println("Are you sure you wanna close this? (y/n)\nYour Vbucks will be returned to you (hehe)");
        while (confirmClose == "") {
            if (!scanner.hasNext()) {
            ...
            if (confirmClose.equals("y")) {
                ...
                System.out.println("Invalid option you noob!\nTry again!");
                confirmClose = "";
            }
        }
    }
